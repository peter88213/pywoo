...
- Fix a regression from v1.35.0 where the "proofing" document export may crash in case no language tags are found.
...
Based on PyWriter v7.11.3

### v1.31.5 Improvements
...
Fix a bug in the StarBasic formatting macros where characters are deleted if no list strokes are found.
...
### v1.12.1 Update text formatting
...
- Improve the Python3 connection to the StarBasic macro (abandon the calling batch file).
